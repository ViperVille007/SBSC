...
def extract_answer(pred_str):
    if 'boxed' in pred_str:
        ...
        pred_st = extract_program_output(pred_str)
        pattern = '-?\d*\.?\d+'
        pred = pred_st
        # pred = re.findall(pattern, pred_st.replace(",", ""))
        # if(len(pred) >= 1):
        #     pred = pred[-1]
        # else: pred = ''
...
def extract_program(result: str, last_only=True):
    ...
    program = re.sub(r"symbols\([^)]+\)", repl, program)
    program = '\n' + program
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    # parse ground truth
    if data_name in ["math", 'ocw', "math_5"]:
        ...
        gt_cot = ""
        gt_ans = example['answer']
    elif data_name in ["camel_algebra", "camel_geometry", "camel_full"]:
        gt_cot = ""
        gt_ans = example['answer']
    else:
...
def run_execute(executor, result, prompt_type, execute=False):
    ...
    elif prompt_type in ["pot", "pal","l2m_pal"] and execute:
