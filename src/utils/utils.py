...
def load_prompt(data_name, prompt_type):
    if data_name in ['gsm-hard', 'svamp', 'tabmwp', 'asdiv', 'mawps']:
        data_name = "gsm8k"
    if data_name in ['math-oai', 'math_5']:
        data_name = "math"
...
def construct_prompt(args, example):
    ...
    if args.use_train_prompt_format:
        full_prompt = f"<|user|>\n{example['question']}\n<|assistant|>\n"
    elif "tora" in args.prompt_type or "sbsc" in args.prompt_type:
...
# to support l2m_pal along with other prompt types
def construct_prompt_l2m(args, example):
    if args.use_train_prompt_format:
        full_prompt = f"<|user|>\n{example['question']}\n<|assistant|>\n"
    elif "tora" in args.prompt_type or "sbsc" in args.prompt_type:
        context = f"Question: {example['question']}\n\nSolution:"
        ...
        context = f"Question: {example['question']}\nAnswer:"
        full_prompt =  context
    elif args.prompt_type == "pal" or args.prompt_type == "l2m_pal":
        context = f"Question: {example['question']}"
        full_prompt =  context
    elif args.prompt_type == "wizard_zs":
        full_prompt = (
        ...
        full_prompt = full_prompt.format(instruction="" + f"Question: {example['question']}\nAnswer:")
    else:
        raise NotImplementedError(args.prompt_type)
    return full_prompt
