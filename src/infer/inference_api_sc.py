...
def parse_args():
    ...
    parser.add_argument("--num_chains", default=7, type=int, help="Number of answer chains to generate for each question")
...
def api_with_func_call(engine,data_name,prompt_type, prompt, max_tokens, temperature, top_p, executor, max_func_call=2, verbose=False):
    '''if n > 1:
...
def process_example(args, example, executor):
    ...
    scores = [math_equal(p, gt_ans, timeout=True) for p in all_predictions]
    
    # Majority voting
    if len(all_predictions) > 0:
        most_common_pred = max(set(all_predictions), key=all_predictions.count)
        is_correct = math_equal(most_common_pred, gt_ans, timeout=True)
    else:
        is_correct = False
...
def main_parallel(args):
            ...
            try:
                ...
                if sample:
                    ...
                    if correct + wrong > 0:
                        ...
                        print(f"Avg Acc: {correct / (correct + wrong):.4f}")
                    print()
            except Exception as e:
    ...
    print()
    if correct + wrong > 0:
        print(f"Final Avg Acc: {correct / (correct + wrong):.4f}")
...
def main(args):
    ...
    for example in tqdm(examples, total=len(examples)):
        sample, is_correct = process_example(args, example, executor)
